...
local a = {
	...
	{
		id = "FIERY_RECLAMATION",
		ui_name = "$perk_ff_reclaim",
		ui_description = "$perkdesc_ff_reclaim",
		ui_icon = "mods/foolish_flame/files/ui_gfx/perk_icons/fiery_reclamation.png",
		perk_icon = "mods/foolish_flame/files/items_gfx/perks/fiery_reclamation.png",
		stackable = STACKABLE_NO,
		func = function (entity_perk_item, entity_who_picked, item_name)
			if EntityHasTag(entity_who_picked, "player_unit") then
                EntityAddTag(entity_who_picked, "ff_fiery_reclamation")
            end
		end,
		func_remove = function(entity_who_picked)
			EntityRemoveTag(entity_who_picked, "ff_fiery_reclamation")
		end
	},
}
